...
var paramView = Backbone.View.extend({
    ...
    render: function(){
        ...
        var html = _.template($(template_name).html(), this.model.toJSON());
...
var subjectView = Backbone.View.extend({
    ...
    render: function(){    
        var html     =  _.template($(this.template).html(), { value : this.value, error : this.error });
        this.$el.html(html);
        return this;
    },
...
var textView = Backbone.View.extend({
    ...
    initialize: function(options){
        ...
        if (!this.control.length)  
            this.render();
        new nicEditor().panelInstance('user_text_adv');
    },

    render: function(){

        var html     =  _.template($(this.template).html(), { value : this.value, text_required : this.text_required });
        this.$el.html(html);
        return this;
    },
...
var photoControlView = Backbone.View.extend({
    ...
    maxLength : 10,
    hints: {
        main: "Главным по умолчанию является первое фото, щелкните по любому фото, чтобы сделать его главным<br>До 10 фотографий с раширениями jpg, png, gif, не более 5мб",
        requires: "До 10 фотографий с раширениями jpg, png, gif, не более 5мб. Фото можно перетащить в эту зону мышкой."
    },
    initialize : function(options){
        var self = this;
        _.extend(this, options);    
...
var categoryDescriptionView = Backbone.View.extend({
    ...
    template : '#template-category-description',
    ...
    render : function(){
        ...
        if (this.text){
            var html     =  _.template($(this.template).html(), {text: this.text});
            this.$el.html(html);
        } else {
            this.$el.html("");
        }
...
var verifyWindowView = Backbone.View.extend({
    ...
    template : "#template-verify-contact-window",
    events : {
...
var contactView = Backbone.View.extend({
    ...
    events : {
        'click .fn-contact-delete-button' : 'remove',
        'click .fn-contact-verify-button' : 'doVerify',
        'change .fn-contact-type' : 'changeType'
    },
    initialize : function (options){
        _.extend(this, options);
        this.render();
...
var contactListView = Backbone.View.extend({
    ...
    events : {
        'click .fn-add-contact-button-text' : 'addContact'
    },
    ...
    initialize : function (options) {
        ...
        this.collection.on("change:status", this.changeStatus, this);
         ...
         _.each(this.$el.find(".contact-cont"), function(item){
            var status = "clear";
    ...
    addItem : function(item){
        item.set("_id", this.collection.indexOf(item));
        var el = undefined;
        if (item.get("id"))
            el = $('#'+item.get("id"));

        this.contacts[item.cid] = new contactView({el : el, model : item, collection: this.collection ,container : this.$el.find(".fn-contacts-container")});           
    },

    removeItem : function(item){
        this.contacts[item.cid].unbind();
        this.contacts[item.cid].$el.remove();           
    },
