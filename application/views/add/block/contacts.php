	...
	<?	foreach ($contacts as $contact):	?>
			...
			<div class="add_form_info contact_type_select_info" id="contact_item_<?=$i?>" data-item-id="<?=$i?>">
				<div class="text">
					...
					{?>
						<option value='<?=$contact_type->id?>' data-validation-type="<?=$contact_type->validation_type?>" data-format="<?=$contact_type->format?>" <?php if ($contact->contact->contact_type_id==$contact_type->id){echo 'selected';}?>><?=$contact_type->name?></option>
					<?
					}?>
					</select>
				</div>
				<div class="values with_right">
						<input id="contact_value_input_<?=$i?>" class='inputs_contact' type="text" value="<?=$contact->contact->contact?>" name="contact_<?=$i?>_value"/>
