...
class Controller_Search extends Controller_Template
{
    ...
    public function action_index()
    {
        ...
        if ($search_info->enable_link_couters) {
            ...
            foreach (array(
                ...
                $city_counter               = Search_Url::getcounters(Domain::get_domain_by_city($city_seo, FALSE, ""), "", array(
                    new Obj(array(
                        "url" => $search_info->canonical_url
                    ))
                ));
        ...
        */
        ...
        $pagination = Pagination::factory(array(
            ...
            'count_in' => 4,
            'limits' => array(
                "25" => Search_Url::get_suri_without_reserved($this->request->query(), array(), array(
                    "limit",
                    "page"
                )),
                "50" => Search_Url::get_suri_without_reserved($this->request->query(), array(
                    "limit" => 50
                ), array(
                    "page"
                )),
                "75" => Search_Url::get_suri_without_reserved($this->request->query(), array(
                    "limit" => 75
                ), array(
                    "page"
                ))
        ...
        $limitList = Pagination::factory(array(
            ...
            'limits' => array(
                "30" => Search_Url::get_suri_without_reserved($this->request->query(), array(), array(
                    "limit",
                    "page"
                )),
                "40" => Search_Url::get_suri_without_reserved($this->request->query(), array(
                    "limit" => 40
                ), array(
                    "page"
                )),
    ...
    public function get_search_info_by_filters()
    {
        ...
        $info->seo_attributes = Seo::get_seo_attributes($this->params_by_uri->get_proper_segments(), $info->search_filters["filters"], $this->params_by_uri->get_category(), $this->domain->get_city());
        
        $info->clean_query_params = $clean_query_params;
        
        $info->query_params_for_js = json_encode(array_merge($this->params_by_uri->get_query_params_without_reserved($this->request->query()), $clean_query_params));
        return $info;
    }
    ...
    public function get_search_info_by_sphinx($search_text)
    {
        ...
        {
            return $a['id'];
        }, $this->getService('Categories')->getCategoryWithChilds($info->category_id, 8));
